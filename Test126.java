...
○ 개요
	...
	sun()사 에서는 자바 프로그램을
	효율적으로 작성 할 수 있더럭 자바 표준 패키지를 제공하며,
	자바 표준 패키지에는 그래픽, 네트워크, 데이터베이스등의
	...
	바로 사용하는 것이 가능하다.
	import 문은 반드시 클래스 설계 구문 전에 선언되어 있어야하며
	형식은 다음과 같은 두 가지가있다.
	...
	만약 패키지를 정적으로  import 하려면 모든 접근이 가능한
	메소드와 멤버 변수들은 접두사(ex- ob, br, sc처럼...)을 붙이지 않고
	사용하는 것이 가능하다. ??????
...
public class Test126	//extends Object
{
	...
	*/
	public static void main(String[] args)
	{
		...
		// --Test126클래스에 toString()메소드가 존재하지 않음에도
		//	에러가 발생하지 않은 상황
		//  → Object로 부터 상속 받음~!!!!!
