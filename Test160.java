...
public class Test160
{
	...
	public static void main(String[] args)
	{
		...
		//벡터 자료구조 v에colors 배열 데이터를 요소를 추가
		...
		//○오름차순 정렬 후 전체 구성
		System.out.print("오른차순 정렬 후 전체 출력 : ");
		for (String s :v )
		{
			System.out.print(s + " ");
		}
		System.out.println();
		//--==>>오른차순 정렬 후 전체 출력 : 노랑 빨강 연두 주황 초록 파랑 하양

		//○ 검색
		//Collections.binarySearch();
		//검색기능 수행.
		//★★★★ 단,오름차순 정렬이 수행되어있는 자료에 대해서만 사용이 가능하다 
		//검색 결과가 존재하지 않을경우 음수를 반환한다.

		int idxBlue = Collections.binarySearch(v,"파랑");
		System.out.printf("파랑 :  %d 번째 인덱스에 위치 \n",idxBlue);
		System.out.println();
		//--==>> 파랑 :  5 번째 인덱스에 위치

		int idxNavi = Collections.binarySearch(v, "남색");
		System.out.printf("남색 :  %d 번째 인덱스에 위치 \n",idxNavi);
		System.out.println();

		//--==>> 남색 :  -1 번째 인덱스에 위치

		//○ 내림차순 정렬 은 별도의 메소드가 없다
