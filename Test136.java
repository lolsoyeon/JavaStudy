...
String은 내용을 변경 할 수 없기때문에
『+』연산자를 사용하게 되면
내부적으로 『StringBuffer』 라는 객체를 생성하며
append() 메소드를 이용하여 문자열에 대한 결합을 수행하게 된다.
따라서 동적인 문자열을 많이 사용하는 Servlet(문자열들을 움직이게하는 프로세서?) 등에서는
되도록이면 String 을 사용하는것보다
StringBuffer 나 char[]을 사용하는것이 효율적일 수 있다.

==> 정적인 문자열을 처리하는 경우에는 주로 String 클래스를 사용
	동적인 문자열을 처리하는 경우에는
	런타임 시 동적으로 처리해 주는 StringfBuffer 클래스를
	사용하는 것이 효율적이다.
...
○ String 객체의 생성 및 특징
	...
	새롭게 메모리 공간을 확보해라 라는 뜻의★『new』연산자를 이용하여 
	객체를 생성하는 것은 내부적으로 의미가 다르다.
*/
...
public class Test136
{
	public static void main(String[] args)
	{
		...
		// 객체 생성한다는 것은 새 메모리를 할당한다는 뜻.  객체니까 주소다~!

		//※ s1과 s2 객체가 참조하는 문자열 상수가 동일한 경우
		//	 문자열이 저장된 기억장소의 영역이 동일하기 때문에
		//   S1 객체와 s2객체는 동일한 기억 장소를 참조하게된다. (s1 == s2)true
		//	 하지만,
		//   s3와 s4는 동일한 영역이 아닌 다른 기억공간을 
		...
		System.out.println("s3.equals(s4) : " + s2.equals(s4));
		System.out.println("s4.equals(s1) : " + s2.equals(s1));
		//--==>> s1.equals(s2) : true
		//		 s2.equals(s3) : true
		//		 s3.equals(s4) : true
		//		 s4.equals(s1) : true
		...
		//==>> 2022-08-08※String 객체의 내용은 불변이다~!~!
